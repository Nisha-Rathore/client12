      ...
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-6 pt-4 md:grid-cols-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} className="relative">
          ...
          <div className="relative rounded-3xl bg-slate-900/70 p-6 ring-1 ring-white/10 shadow-2xl">
            <div className="mb-5">
              <p className="text-sm text-slate-300">Start your free trial</p>
              <p className="text-lg font-semibold text-white">No credit card needed</p>
            </div>
            ...
            <form onSubmit={submit} className="grid grid-cols-1 gap-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 ring-1 ring-white/10 focus-within:ring-teal-300/40">
                    ...
                    <input value={form.name} onChange={(e)=>update("name", e.target.value)} placeholder="Alex Carter" className="w-full bg-transparent py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 ring-1 ring-white/10 focus-within:ring-teal-300/40">
                    ...
                    <input value={form.phone} onChange={(e)=>update("phone", e.target.value)} placeholder="+91 90000 00000" className="w-full bg-transparent py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 ring-1 ring-white/10 focus-within:ring-teal-300/40">
                  ...
                  <input value={form.email} onChange={(e)=>update("email", e.target.value)} type="email" placeholder="you@gyms.com" className="w-full bg-transparent py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 ring-1 ring-white/10 focus-within:ring-teal-300/40">
                    ...
                    <input value={form.pass} onChange={(e)=>update("pass", e.target.value)} type={show?"text":"password"} placeholder="At least 8 characters" className="w-full bg-transparent py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none" />
                    <button type="button" onClick={()=>setShow(!show)} className="p-1 text-slate-400 hover:text-slate-200">{show? <EyeOff className="h-4 w-4"/>:<Eye className="h-4 w-4"/>}</button>
                  </div>
                  {/* strength bar */}
                  <div className="mt-2">
                    ...
                    <div className="mt-1 h-2.5 w-full overflow-hidden rounded-full bg-slate-700/60">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2 rounded-xl bg-slate-800/80 px-3 ring-1 ring-white/10 focus-within:ring-teal-300/40">
                    <Lock className="h-4 w-4 text-slate-400" />
                    <input value={form.confirm} onChange={(e)=>update("confirm", e.target.value)} type={show?"text":"password"} placeholder="Re enter password" className="w-full bg-transparent py-2 text-sm text-white placeholder:text-slate-400 focus:outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select value={form.plan} onChange={(e)=>update("plan", e.target.value)} className="mt-1 w-full rounded-xl bg-slate-800/80 px-3 py-2 text-sm text-slate-200 ring-1 ring-white/10 focus:outline-none">
                ...
                <div className="rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
                  <p className="text-xs font-semibold text-white">Perks with trial</p>
                  <ul className="mt-1 space-y-1 text-[11px] text-slate-300">
                    <li className="flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-teal-300"/> 1 admin seat</li>
                    <li className="flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-teal-300"/> Unlimited members</li>
                    <li className="flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5 text-teal-300"/> Class scheduling</li>
                  </ul>
                </div>
              </div>

              {error && (
                <div className="rounded-lg bg-rose-400/10 p-2 text-xs text-rose-200 ring-1 ring-rose-300/30 flex items-center gap-2"><AlertTriangle className="h-4 w-4"/> {error}</div>
              )}
              {ok && (
                <div className="rounded-lg bg-emerald-400/10 p-2 text-xs text-emerald-200 ring-1 ring-emerald-300/30 flex items-center gap-2"><CheckCircle2 className="h-4 w-4"/> {ok}</div>
              )}

              <div className="flex flex-wrap items-center justify-between gap-3">
                <label className="flex items-center gap-2 text-xs text-slate-300">
                  <input checked={agree} onChange={(e)=>setAgree(e.target.checked)} type="checkbox" className="h-3.5 w-3.5 rounded border-slate-600 bg-slate-800 focus:ring-0" />
                  I agree to the <a href="#terms" className="font-semibold text-teal-200">Terms</a> and <a href="#privacy" className="font-semibold text-teal-200">Privacy</a>
                </label>
                <label className="flex items-center gap-2 text-xs text-slate-300">
                  <input checked={promo} onChange={(e)=>setPromo(e.target.checked)} type="checkbox" className="h-3.5 w-3.5 rounded border-slate-600 bg-slate-800 focus:ring-0" />
              ...
              <div className="relative py-3 text-center text-xs text-slate-400">
                <span className="relative z-10 bg-slate-900/70 px-3">or sign up with</span>
              ...
              <p className="pt-2 text-center text-xs text-slate-400">
                Already have an account? <a href="#signin" className="font-semibold text-teal-200">Sign in</a>
              </p>
            </form>
          </div>
        </motion.div>
