         ...
         <Layout>
            ...
            {stats.map((s) => (
              <motion.div key={s.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-2xl bg-white shadow-sm flex items-center justify-between">
                <div>
                  <div className="text-xs text-teal-400">{s.title}</div>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white p-4 rounded-2xl shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-xs text-teal-500">Last 30 days</div>
              </div>

              {/* Simple SVG chart (sparkline + bars) */}
              <div className="w-full h-40 mb-4">
                <svg className="w-full h-full" viewBox="0 0 600 160" preserveAspectRatio="none">
                  <polyline fill="none" stroke="#14b8a6" strokeWidth="3" points="0,120 80,100 160,80 240,95 320,60 400,72 480,40 560,55 600,30" />
                </svg>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-2">
                    {orders.map((o) => (
                        <div key={o.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-teal-50">
                        <div>
                          <div className="text-sm font-medium">Order #{o.id}</div>
                          <div className="text-xs text-teal-500">{o.customer} • {o.date}</div>
                        </div>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {topProducts.map((p) => (
                      <li key={p.id} className="p-3 rounded-lg hover:bg-teal-50 flex items-center justify-between">
                        <div>
                          <div className="font-medium">{p.title}</div>
                          <div className="text-xs text-teal-500">Sold: {p.sold}</div>
                        </div>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-3">
                <button className="flex items-center justify-between p-3 rounded-lg hover:bg-teal-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-teal-500">Create a new product listing</div>
                    </div>
                  </div>
                  <div className="text-xs text-teal-400">Ctrl + P</div>
                </button>
                ...
                <button className="flex items-center justify-between p-3 rounded-lg hover:bg-teal-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-teal-500">Detailed sales & analytics</div>
                    </div>
                  </div>
                  <div className="text-xs text-teal-400">Enter</div>
                </button>
                ...
                <button className="flex items-center justify-between p-3 rounded-lg hover:bg-teal-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-teal-500">Email/SMS campaign</div>
                    </div>
                  </div>
          ...
          <section className="mt-6 bg-white p-4 rounded-2xl shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-sm text-teal-500">Showing latest 50 orders</div>
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="text-xs text-teal-500 border-b">
                  <tr>
                ...
                <tbody className="divide-y">
                  {orders.map(o => (
                    <tr key={o.id} className="hover:bg-teal-50">
                      <td className="py-3">#{o.id}</td>
                      <td className="py-3">{o.customer}</td>
                      <td className="py-3">₹{o.amount}</td>
                      <td className="py-3">{o.status}</td>
                      <td className="py-3">{o.date}</td>
                      <td className="py-3"><button className="text-sm px-3 py-1 rounded-md bg-teal-100 hover:bg-teal-200">View</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
